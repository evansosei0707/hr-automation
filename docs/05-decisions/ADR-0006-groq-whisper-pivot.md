...
- No published benchmark for Ghanaian Pidgin or West African accented English on any Whisper variant. Researcher noted this is inferential — both Whisper-large-v3 and OpenAI's `gpt-4o-mini-transcribe` descend from the same architecture, so practical differences are likely small but unverified. **The 2-week post-launch calibration window is the empirical answer here.** During calibration, Workflow A's voice-note pass is fully human-reviewed before any auto-action; we'll see real Ghanaian-Pidgin transcription quality on real candidate audio and tune confidence thresholds against that.
